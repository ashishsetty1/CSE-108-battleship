...
function App() {
  ...
  useEffect(() => {
    ...
    socket.on("attack_result", (data) => {
      ...
      if (data.sunk_ship) {
        newLogs.unshift(
          `💥 ${data.attacker} destroyed ${data.defender}'s ${data.sunk_ship}!`
        );
      }
      ...
      if (data.sunk_ship) {
        setMessage(`💥 ${data.sunk_ship} destroyed!`);
      }
  ...
  const isTempCell = (row, col) => {
    return currentShipCells.some(([r, c]) => r === row && c === col);
  };

  const validateTempSelection = (newCells, shipSize) => {
    if (newCells.length > shipSize) {
      return "You selected too many cells for this ship.";
    }
    ...
    if (rows.size > 1 && cols.size > 1) {
      return "Ship must be placed in a straight row or column.";
    }
      ...
      if (rows.size === 1) {
        ...
        for (let i = 1; i < sortedCols.length; i++) {
          if (sortedCols[i] !== sortedCols[i - 1] + 1) {
            return "Ship cells must be connected.";
          }
        }
      }
  ...
  const selectShipCell = (row, col) => {
    ...
    if (isExistingShipCell(row, col)) {
      alert("You cannot overlap ships.");
      return;
    }

    if (isTempCell(row, col)) {
      setCurrentShipCells((prev) =>
  ...
  const attackSquare = (row, col) => {
    ...
    if (enemyBoard[row][col] === "hit" || enemyBoard[row][col] === "miss") {
      alert("You already attacked that square.");
      return;
    }
  ...
  const renderBoard = (board, onClick, showTemp = false) => {
    return (
      <div className="board-wrap">
        ...
        {LETTERS.map((letter) => (
          <div className="coord-label" key={letter}>
            {letter}
          </div>
        ))}
    ...
    <>
      <style>{`
        * { box-sizing: border-box; }
        ...
        .hit {
          background: radial-gradient(circle, #fef2f2, #ef4444 45%, #7f1d1d 100%);
        }
        ...
        @media (max-width: 700px) {
          ...
          .board-wrap {
            grid-template-columns: 22px repeat(10, 27px);
            gap: 3px;
          }
            ...
            <div className="players">
              {players.map((p, i) => (
                ...
                >
                  <strong>{p}</strong>
                  <div>
                    {phase === "game" && i === turn ? "Current Turn" : "Player"}
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="panel">
          {phase === "lobby" && (
            <div className="lobby-card">
              ...
              <div style={{ margin: "18px 0", color: "#94a3b8" }}>or</div>
          ...
          {phase === "placing" && (
            <>
              ...
              {currentShip ? (
                <>
                  <h3>
                    Current Ship: {currentShip.name} ({currentShip.size} cells)
                  </h3>
                  <p className="legend">
                    Click each square manually. Selected cells must be connected
                    in one straight horizontal or vertical line.
                  </p>
                  <p>
                    Selected: <strong>{currentShipCells.length}/{currentShip.size}</strong>
                  </p>
                </>
              ) : (
              ...
              <div className="actions">
                ...
                <button className="secondary-btn" onClick={clearCurrentShip}>
                  Clear Selection
                </button>
                <button className="danger-btn" onClick={finishPlacement}>
                  Done Placing
                </button>
              </div>
            </>
              ...
              <div className="compact-game-layout">
                ...
                <div className="battle-log wide-log">
                  ...
                  <div className="fleet-grid">
                    {players.map((player) =>
                      SHIPS.map((ship) => {
                        const playerSunkShips = sunkShips[player] || [];
                        const isSunk = playerSunkShips.includes(ship.name);

                        return (
                          <div
                            key={`${player}-${ship.name}`}
                            className={isSunk ? "fleet-item sunk" : "fleet-item"}
                          >
                            <strong>{player}</strong> — {ship.name}:{" "}
                            {isSunk ? "💥 Sunk" : "✅ Afloat"}
                          </div>
                        );
                      })
